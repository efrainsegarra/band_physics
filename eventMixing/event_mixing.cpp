...
int main(int argc, char ** argv){
	...
	double e_theta	[inTree_e->GetEntries()/1000];
	double e_phi	[inTree_e->GetEntries()/1000];
	double e_mom	[inTree_e->GetEntries()/1000];
	double e_beam	[inTree_e->GetEntries()/1000];
	...
	for( int electron = 0 ; electron < inTree_e->GetEntries() ; electron++ ){
		...
		if( electron > inTree_e->GetEntries()/1000 ) break;
	...
	for( int neutron = 0 ; neutron < inTree_n->GetEntries() ; neutron++ ){
		...
		while( nPairs < 100 && nFails < 1000 ){
			int electron = myRand->Rndm() * inTree_e->GetEntries()/1000;
			...
			double beta = (1./ToF) * (1./cAir) * (100./1);
			...
			if( W2 > 2*2 && Q2 > min_Q2 && Q2 < max_Q2 && CosTheta_nq > min_CosTheta_nq && CosTheta_nq < max_CosTheta_nq ){
				// Valid pair
				nFails = 0;
				nPairs++;
