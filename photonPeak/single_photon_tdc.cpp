...
int main(int argc, char ** argv){
	...
	for( int i = 2 ; i < argc ; i++ ){
		...
		double meantimeFadc, meantimeTdc, STTime, dL, ToF;
		...
		for( int ev = 0 ; ev < inTree->GetEntries() ; ev++ ){
			if( ev % 1000000 == 0 ) cout << "\ton event " << ev << "\n";
				...
				meantimeTdc = 0; STTime = 0; dL = 0; ToF = 0;
				...
				//double tof = (meantimeFadc - STTime - 41.9197 - dL/30.);
	...
	ofstream out_file;
	out_file.open("photon_params_tdc.txt");
